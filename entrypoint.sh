...
for i in {1..30}; do
    if /opt/mssql-tools18/bin/sqlcmd -S APT045CD119FX9N\MSSQLSERVER2025 -U sa -P "${MSSQL_SA_PASSWORD}" -C -Q "SELECT 1" &> /dev/null; then
...
/opt/sqlpackage/sqlpackage /Action:Publish \
    ...
    /TargetServerName:APT045CD119FX9N\MSSQLSERVER2025 \
